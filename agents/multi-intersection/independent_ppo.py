...
try:
    ...
    raise ImportError("TraCI not found. Make sure SUMO is installed and SUMO_HOME is set correctly. Original error: {}".format(e))

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print('Device:', device)
print('SUMO_HOME:', os.environ.get('SUMO_HOME'))

#change paths if needed (check README.md for details)
...
class IndependentPPOAgent:
    def __init__(self, num_agents, obs_dim, action_dim):
        ...
        self.critics = nn.ModuleList([CriticNet(obs_dim).to(device) for _ in range(num_agents)])

        self.actor_optimizers = [optim.Adam(actor.parameters(), lr=POLICY_LR) for actor in self.actors]
        self.critic_optimizers = [optim.Adam(critic.parameters(), lr=VALUE_LR) for critic in self.critics]
    ...
    def save(self, path):
        checkpoint = {
            ...
            'critic_optim_states': [opt.state_dict() for opt in self.critic_optimizers]
        }
    ...
    def load(self, path):
        ...
        for i, critic in enumerate(self.critics):
            critic.load_state_dict(checkpoint['critic_states'][i])
        ...
        for i, opt in enumerate(self.critic_optimizers):
            opt.load_state_dict(checkpoint['critic_optim_states'][i])
    ...
    def select_actions(self, obs_dict):
        ...
        for i, (tls, obs) in enumerate(sorted(obs_dict.items())):
            ...
            v = self.critics[i](obs_tensor.unsqueeze(0)).squeeze(0)
            values[tls] = v
    ...
    def update(self, rollout):
        ...
        for _ in range(PPO_EPOCHS):
            for start in range(0, batch_size, max(1, batch_size//MINI_BATCHES)):
                end = start + max(1, batch_size//MINI_BATCHES)
                ...
                for i in range(num_agents):
                    ...
                    entropy = dist.entropy()
                    value = self.critics[i](mb_obs[:, i, :])
                    log_probs.append(log_prob)
                    entropies.append(entropy)
                    values_pred.append(value)
                log_probs = torch.stack(log_probs, dim=1)
                entropies = torch.stack(entropies, dim=1)
                values_pred = torch.stack(values_pred, dim=1)
...
def train():
    ...
    ippo_agent = IndependentPPOAgent(num_agents, obs_dim, action_dim)

    resume_path = "checkpoints/ippo_latest.pth"
    if os.path.exists(resume_path):
        ippo_agent.load(resume_path)
    ...
    for ep in range(NUM_EPISODES):
        ...
        while not done and step < MAX_EPISODE_STEPS:
            ...
            actions, log_probs, values = ippo_agent.select_actions(obs)
        ...
        if (ep + 1) % 1000 == 0:
            save_path = f"checkpoints/ippo_ep{ep+1}.pth"
            ippo_agent.save(save_path)
            ippo_agent.save("checkpoints/ippo_latest.pth") 
            metrics = {
    ...
    ippo_agent.save("checkpoints/ippo_final_model.pth")
    env.close_sumo()
    ...
    plt.plot(reward_gauss, linewidth=2, label=f'Gaussian σ={sigma}')
    ...
    plt.figure(figsize=(10,5))
    plt.plot(episode_queue_lengths, alpha=0.3, label='Raw')
    plt.plot(queue_gauss, linewidth=2, label=f'Gaussian σ={sigma}')
    ...
    plt.figure(figsize=(10,5))
    plt.plot(episode_waiting_times, alpha=0.3, label='Raw')
    plt.plot(wait_gauss, linewidth=2, label=f'Gaussian σ={sigma}')
